...
new #[Title('Master Rumah')] class extends Component {
    ...
    {
        $rumah = Rumah::with('proyek')->findOrFail($id);
        $unit = trim(($rumah->blok ?? '').'-'.($rumah->nomor_unit ?? ''), '-');
        $this->riwayatTitle = "Riwayat Unit {$unit}"
            .($rumah->proyek?->nama_proyek ? ' · '.$rumah->proyek->nama_proyek : '');

        $logs = \Spatie\Activitylog\Models\Activity::query()
            ->where('subject_type', Rumah::class)
            ->where('subject_id', $id)
            ->with('causer')
...
<section class="w-full">
    <div class="mx-auto max-w-7xl px-4 py-6 sm:px-6 lg:px-8">
        ...
        {{-- ═══════════════ MODAL RIWAYAT UNIT ═══════════════ --}}
        <flux:modal name="riwayat-unit" class="md:w-3xl">
            <div class="space-y-4">
                <div class="flex items-start gap-3 border-b border-zinc-200 pb-3 dark:border-zinc-700">
                    ...
                    <div class="min-w-0 flex-1">
                        ...
                        <flux:subheading>{{ __('Timeline perubahan data unit (maksimal 100 entri terbaru).') }}</flux:subheading>
                    </div>
                </div>
                ...
                <div class="max-h-[60vh] overflow-y-auto">
                    ...
                    @empty
                        <div class="py-12 text-center text-sm text-zinc-400">
                            {{ __('Belum ada riwayat perubahan untuk unit ini.') }}
                        </div>
                    @endforelse
                </div>
